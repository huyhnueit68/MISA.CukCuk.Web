...
$(document).ready(function() {
    ...
    /* event on click select option*/
    $(".drop-down .dr-item").on("click", function(e) {
        let $items = $(this),
            value = $items.find(".text").text(), //get text of option select
            $parent = $items.parents('.content-control'),
            $cbbValue = $parent.find("#cbbPosition");

        //set unaction for all
        $items.parents(".drop-down").find(".dr-item").removeClass("active");

        //set active for input choose
        $cbbValue.val(value);

        //add action class for element
    ...
    $('.input-field .btn-clear').on("click", function() {
        ...
        //xóa giá trị ô input
        $parent.find("#cbbPosition").val("");
        //Ẩn nút clear
        $item.hide();
    });

    /**
     * hidden drop dow when click out combobox
     */
    $(document).on("click", function(e) {
        let $el = $(e.target),
            isArrow = $el.data("arrow");
